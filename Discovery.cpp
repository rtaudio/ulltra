...
Discovery::NodeDevice Discovery::NodeDevice::any;
...
const Discovery::NodeDevice &Discovery::getNode(const addrinfo &addr) const
{
    ...
    return Discovery::NodeDevice::any;
}
...
Discovery::NodeDevice::NodeDevice(const std::string &host) {
    ...
    if( (r = getaddrinfo(host.c_str(), NULL, &addr, &res)) < 0) {
...
const struct sockaddr_storage Discovery::NodeDevice::getAddr(int port) const {
	struct sockaddr_storage s = saddr;
...
std::ostream& operator<<(std::ostream &strm, const Discovery::NodeDevice &nd) {
	return strm << nd.name << " (id " << nd.id << ", host " << (nd.addr.ai_canonname ? nd.addr.ai_canonname : "") << ")";
}
